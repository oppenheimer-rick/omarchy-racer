// RacerModel.js — OutRun-style 3D Road Racer Engine
// Authentic port from jakesgordon/javascript-racer
...
function buildRoad(state) {
  ...
  // Add Billboards and Palm Trees exactly as in the original game
  addSprite(state, 20, SPRITES.BILLBOARD07, -1.1);
  addSprite(state, 40, SPRITES.BILLBOARD06, -1.1);
  addSprite(state, 60, SPRITES.BILLBOARD08, -1.1);
  addSprite(state, 80, SPRITES.BILLBOARD09, -1.1);
  addSprite(state, 100, SPRITES.BILLBOARD01, -1.1);
  addSprite(state, 120, SPRITES.BILLBOARD02, -1.1);
  addSprite(state, 140, SPRITES.BILLBOARD03, -1.1);
  addSprite(state, 160, SPRITES.BILLBOARD04, -1.1);
  addSprite(state, 180, SPRITES.BILLBOARD05, -1.1);
  ...
  for (var n = 10; n < 200; n += 4 + Math.floor(n / 100)) {
    addSprite(state, n, SPRITES.PALM_TREE, 1.15 + Math.random() * 0.2);
    addSprite(state, n, SPRITES.PALM_TREE, -1.25 - Math.random() * 0.2);
  }

  for (var m = 250; m < 1000; m += 5) {
    addSprite(state, m, SPRITES.TREE1, -1.4 - Math.random() * 0.5);
    addSprite(state, m, SPRITES.TREE2, 1.4 + Math.random() * 0.5);
  }
...
function resetCars(state) {
  ...
  for (var i = 0; i < 45; i++) {
    ...
    var z = Math.floor(Math.random() * (state.segments.length - 50) + 20) * state.segmentLength;
    var sprite = carSprites[Math.floor(Math.random() * carSprites.length)];
    var speed = state.maxSpeed / 4 + Math.random() * (state.maxSpeed / 2.5);
